...
class AmbiguousPostError(RuntimeError):
    """The client cannot know whether Zepp committed the submitted steps."""
...
def post_fake_brand_data(step, app_token, userid, data_date=None):
    t = get_time()

    today = data_date or get_beijing_time().strftime("%Y-%m-%d")
    ...
    if response.status_code != 200:
        raise AmbiguousPostError("请求修改步数结果不明：%d" % response.status_code)
    response = response.json()
    message = response["message"]
    if message == "success":
        return True, message
